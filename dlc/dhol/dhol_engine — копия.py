...
def apply_meat_and_fat(sig, fs, articulation, strike_force, saturation=0.0, skin_mat=None, shell_mat=None, shell_attack=0.5, shell_sustain=1.0, membrane_snap=1.0):
    ...
    if articulation in ["tek_A", "tek_B"]:
        ...
        # ГИБРИД: 450 мс хвост кожи (было 250, в клэпе 650)
        click_decay = 0.45  
        snap_boost = 5.7    
    else:
        click_decay = 0.04
...
def apply_internal_bells(membrane_signal, acceleration_signal, fs, material_name="brass", 
    ...
    """
    Продвинутая модель внутренних колокольчиков (Shkhshkhan).
    Учитывает дисперсию, фрагментацию, микро-отскоки множества колец и 
    жестко коррелирует с Round Robin для уникальности каждого сэмпла.
    """
    from config.materials import MATERIAL_PHYSICS
    ...
    gains = [1.0, 0.75, 0.45, 0.20]
    base_q = np.clip(0.5 / max(1e-5, loss), 10.0, 400.0)
    
    abs_acc = np.abs(acceleration_signal)
    main_excitation = np.zeros_like(abs_acc)
    
    threshold = 0.0004 * np.sqrt(rho)
    active_mask = abs_acc > threshold
    main_excitation[active_mask] = abs_acc[active_mask] - threshold
    
    num_samples = len(membrane_signal)
    hit_idx = int(np.argmax(abs_acc)) if np.max(abs_acc) > 0 else 0
    t_decay = np.maximum(0, np.arange(num_samples) - hit_idx) / fs
    
    bounce_decay_time = 0.15 + 0.3 * strike_force
    bounce_env = np.exp(-t_decay / bounce_decay_time) * strike_force
    ...
    for bell_idx in range(num_bells):
        ...
        q_bell = base_q * rng.uniform(0.7, 1.3)
    ...
    b_hp, a_hp = butter(2, 1800.0 / (fs / 2.0), btype='high')
    bell_output = lfilter(b_hp, a_hp, bell_output)
            
    dry_env = lfilter([0.05], [1.0, -0.95], np.abs(membrane_signal))
    max_env = np.max(dry_env) + 1e-9
    duck_factor = 1.0 - 0.9 * np.clip(dry_env / max_env, 0.0, 1.0)
    bell_output *= duck_factor
    
    max_bell = np.max(np.abs(bell_output))
    if max_bell > 0:
        bell_output /= max_bell
        bell_output *= np.clip(np.max(main_excitation) * 3.0, 0.0, 1.0)
        
    return membrane_signal + bell_output * mix
...
def synthesize_dhol_strike(
    ...
    for step in range(max_steps):
        ...
        for sub in range(M):
            ...
            if has_tactile:
                ...
                if articulation == "chapa" and step < int(0.08 * fs):
                    t_curr = step / fs
                    env = np.exp(-t_curr / 0.050)
                    if t_curr < 0.003:
                        env *= (t_curr / 0.003)
                    slap_fric_A_step = env * (strike_force ** 1.1) * 22.0
    ...
    if articulation != "bass_slide":
        # Удлиняем песочный хвост для Chapa, делая его длиннее Тэка!
        if articulation == "chapa":
            tact_decay = 0.50
        elif articulation in ["tek_A", "tek_B"]:
            tact_decay = 0.30
